...
class ShiftConv2d1(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        self.weight = nn.Parameter(torch.zeros(inp_channels, 1, 3, 3), requires_grad=False)## inp_channels种 1通道 的3*3卷积核
        self.n_div = 5
        g = inp_channels // self.n_div #25
        self.weight[0*g:1*g, 0, 1, 2] = 1.0 ## left 0:25,0,1,2
        self.weight[1*g:2*g, 0, 1, 0] = 1.0 ## right 25:50,0,1,0
    ...
    def forward(self, x):
        y = F.conv2d(input=x, weight=self.weight, bias=None, stride=1, padding=1, dilation=1, groups=self.inp_channels)
        y = self.conv1x1(y) 
        return y

# class ShiftConv2d3(nn.Module):
#     def __init__(self, inp_channels, out_channels):
#         super(ShiftConv2d3, self).__init__()    
#         self.inp_channels = inp_channels
#         self.out_channels = out_channels

#         self.weight = nn.Parameter(torch.zeros(inp_channels,1,5,5), requires_grad=False)## inp_channels种1通道的3*3卷积核
#         self.n_div = 5
#         g = inp_channels // self.n_div #25
#         self.weight[0*g:1*g,0,1, 4] = 1.0 ## left 0:25,0,1,2
#         # self.weight[1*g:2*g, 0, 1, 0] = 1.0 ## right 25:50,0,1,0
#         # self.weight[2*g:3*g, 0, 2, 1] = 1.0 ## up
#         # self.weight[3*g:4*g, 0, 0, 1] = 1.0 ## down
#         # self.weight[4*g:, 0, 1, 1] = 1.0 ## identity     
    ...
    def forward(self, x, prev_atns = None):
        ...
        if prev_atns is None:
            for idx, x_ in enumerate(xs):
                ...
                if self.shifts > 0:
                    x_ = torch.roll(x_, shifts=(-wsize//2, -wsize//2), dims=(2,3))#Figure.2.c shifted window mechanism
                ...
                atn = (q @ q.transpose(-2, -1))#[16,16,16] 
                atn = atn.softmax(dim=-1)
                y_ = (atn @ v)#[16,16,64]
                y_ = rearrange(
                    y_, '(b h w) (dh dw) c-> b (c) (h dh) (w dw)', #bhw 窗口数，dhdw窗大小，c通道数
...
class ELAB(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1 + self.shared_depth):           
            if i == 0: 
                x = self.modules_lfe['lfe_{}'.format(i)](x) + x  ##LFE
                y, atn = self.modules_gmsa['gmsa_{}'.format(i)](x, None) ##GMSA
                x = y + x
            else:
                x = self.modules_lfe['lfe_{}'.format(i)](x) + x ##LFE
                y, atn = self.modules_gmsa['gmsa_{}'.format(i)](x, atn) ##GMSA
                x = y + x
        return x

# if __name__ == '__main__':
#     torch.manual_seed(0)
#     input = torch.randn(1,129,256,256)
#     shift_conv1 = ShiftConv2d3(129, 256)
#     out1 = shift_conv1(input)
#     print(out1.shape)